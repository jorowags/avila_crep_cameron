...
## We are using trackdown for this paper - Instructions on trackdown here: https://cran.r-project.org/web/packages/trackdown/vignettes/trackdown-workflow.html 
```
...
## one participant wrote "19 2004" and that means they are 19
data$age[data$CASE == 527] <- 19
...
data$gender <- gsub("girl","female", data$gender)
data$gender <- gsub("I identify as a cisgendered woman.","female", data$gender)
data$gender <- gsub("she/her","female", data$gender)
data$gender <- gsub("Woman","female", data$gender)
data$gender <- gsub("woman","female", data$gender)
...
data$race <- gsub("White/White", "White", data$race)
data$race <- gsub("African-American", "Black", data$race)
data$race <- gsub("black", "Black", data$race)
data$race <- gsub("Hispanic ", "Hispanic", data$race)
data$race <- gsub("histpanic ", "Hispanic", data$race)
data$race <- gsub("latino ", "Hispanic", data$race)
data$race <- gsub("White ", "White", data$race)
data$race <- gsub("White/ mexican", "Multiracial", data$race)
data$race <- gsub("Whiteis right", "White", data$race)
data$race <- gsub("Native American/Hispanic", "Multiracial", data$race)
...
### rewrite below -- we now *do* find a significant correlation between deck choice and perceived effort
...
### we now do have a significant main effect of time
...
### this last test is no longer significant
